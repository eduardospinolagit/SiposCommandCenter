...
export const useAdminWaStore = defineStore('adminWa', () => {
  const templates   = ref([])
  ...
  const unreadCounts = reactive(JSON.parse(localStorage.getItem('slac-admin-unread-counts') || '{}'))

  function _persistUnread() {
    localStorage.setItem('slac-admin-unread-counts', JSON.stringify({ ...unreadCounts }))
  }
  ...
  function _getLastSeen() {
    try { return JSON.parse(localStorage.getItem('slac-admin-last-seen') || '{}') } catch { return {} }
  }
  ...
  async function loadConfig() {
    const { data } = await sb
      ...
      .eq('user_id', uid())
      .in('chave', ['admin_wa_config', 'admin_script_base'])
    const waConf = data?.find(r => r.chave === 'admin_wa_config')
    const script = data?.find(r => r.chave === 'admin_script_base')
  ...
  async function saveConfig(instanceId, script) {
    ...
    await Promise.all([
      sb.from('configuracoes').upsert({
        id: userId + '_admin_wa_config', user_id: userId,
        chave: 'admin_wa_config', valor: { instance_id: instanceId },
        updated_at: new Date().toISOString()
      }, { onConflict: 'id' }),
      sb.from('configuracoes').upsert({
        id: userId + '_admin_script_base', user_id: userId,
        chave: 'admin_script_base', valor: { texto: script },
        updated_at: new Date().toISOString()
      }, { onConflict: 'id' }),
    ])
  ...
  async function loadSdrConfig({ includeChats = true } = {}) {
    const chaves = includeChats ? ['admin_sdr_config', 'admin_sdr_chats'] : ['admin_sdr_config']
    const { data } = await sb
      .from('configuracoes').select('chave, valor')
      .eq('user_id', uid())
      .in('chave', chaves)
    if (!data) return
    const cfgRow   = data.find(r => r.chave === 'admin_sdr_config')
    const chatsRow = data.find(r => r.chave === 'admin_sdr_chats')
  ...
  async function saveSdrConfig() {
    ...
    await sb.from('configuracoes').upsert({
      id: userId + '_admin_sdr_config', user_id: userId,
      chave: 'admin_sdr_config', valor: { ...sdrConfig.value },
      updated_at: new Date().toISOString(),
    }, { onConflict: 'id' })
  ...
  async function toggleSdrChat(lead) {
    ...
    const userId = uid()
    await sb.from('configuracoes').upsert({
      id: userId + '_admin_sdr_chats', user_id: userId,
      chave: 'admin_sdr_chats', valor: { ...sdrChats },
      updated_at: new Date().toISOString(),
    }, { onConflict: 'id' })
  ...
  async function loadFuAutoConfig() {
    try {
      const { data } = await sb.from('configuracoes').select('valor')
        .eq('user_id', uid()).eq('chave', 'admin_fu_auto_chats').maybeSingle()
  ...
  async function saveFuAutoConfig() {
    try {
      const userId = uid()
      await sb.from('configuracoes').upsert({
        id: userId + '_admin_fu_auto_chats', user_id: userId,
        chave: 'admin_fu_auto_chats', valor: { ...fuAutoChats },
        updated_at: new Date().toISOString()
      }, { onConflict: 'id' })
    } catch {}
  }
